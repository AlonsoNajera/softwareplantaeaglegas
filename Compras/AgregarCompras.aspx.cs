...
namespace SoftwarePlantas.Compras
{
    public partial class AgregarCompras : System.Web.UI.Page
    {
        ...
        private void CargarTanques()
        {
            ...
            {
                string query = "SELECT Tanque, NomProd FROM Tanques";
                SqlDataAdapter da = new SqlDataAdapter(query, conn);
                DataTable dt = new DataTable();
                da.Fill(dt);

                ddlTanque.DataSource = dt;
                ddlTanque.DataTextField = "NomProd";
                ddlTanque.DataValueField = "Tanque";
                ddlTanque.DataBind();

                ddlTanque.Items.Insert(0, new System.Web.UI.WebControls.ListItem("Seleccione un tanque", ""));
            }
        }
        ...
        protected void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            string idTanque = ddlTanque.SelectedValue;
            string litros = txtLitros.Text;

            if (string.IsNullOrEmpty(fecha) || string.IsNullOrEmpty(idTanque) || string.IsNullOrEmpty(litros))
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "faltandatos",
                    "Swal.fire('Error', 'Todos los campos son obligatorios', 'error');", true);
                return;
            }
            string instanciaConnectionStringInsert = Session["instanciaSeleccionada"]?.ToString();

            if (string.IsNullOrEmpty(instanciaConnectionStringInsert))
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "error",
                    "Swal.fire('Error', 'Instancia no seleccionada.', 'error');", true);
                return;
            ...
            using (SqlConnection conn = new SqlConnection(instanciaConnectionStringInsert))
            {
               
                SqlCommand command = new SqlCommand("sp_ComprasPlantas", conn);
                command.CommandType = CommandType.StoredProcedure;

                // Agregar parámetros
                command.Parameters.AddWithValue("@Tanque", idTanque);
                command.Parameters.AddWithValue("@VolumenDocumentado", litros);
            ...
            // Mensaje de éxito
            ScriptManager.RegisterStartupScript(this, GetType(), "registroOk", @"
                Swal.fire('Guardado', 'Registro insertado correctamente', 'success');
                setTimeout(function() {
                    document.getElementById('" + txtFecha.ClientID + @"').value = '" + DateTime.Now.ToString("yyyy-MM-dd") + @"';
                    document.getElementById('" + ddlTanque.ClientID + @"').selectedIndex = 0;
                    document.getElementById('" + txtLitros.ClientID + @"').value = '';
                }, 1);", true);
        }
    }
}